...
def gym_generate_subscription(request, schema_name, customer_id):
    ...
    with schema_context(schema_name):
        ...
        for i in range(months):
            ...
            # Check if subscription already exists for that month
            if GymSubscription.objects.filter(customer=customer, month=target_month, year=target_year).exists():
                continue

            # Due date: membership start day or fallback to generation day
