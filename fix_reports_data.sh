...
# fix_reports_data.sh - Fixes reports view to show payment data correctly
...
# Backup
cp "$VIEWS_FILE" "${VIEWS_FILE}.bak_reports_data"
echo "✅ Backup created."

# ----------------------------------------------------------------------
# Replace the entire reports function with a clean, working version
# ----------------------------------------------------------------------
...
# New corrected reports function
new_func = '''def reports(request, schema_name):
    ...
    # Default: if any payments exist → all time, else last 6 months
    ...
    if quick_filter == 'today':
        ...
        # No filter: show all time if there are any payments, else last 6 months
    ...
    # Step 2: Build the queryset inside the tenant schema
    with schema_context(schema_name):
        ...
        # Pending fees (always all time, not filtered by date)
...
# Ensure there is no duplicate reports function left
if new_content.count("def reports(") > 1:
    # Aggressive removal: keep only the first occurrence
...
print("✅ reports function replaced and cleaned up.")
PYTHON_SCRIPT
